...
const int WIDTH[12] = { 0, 0, 3, 5, 7, 7, 8, 8, 8, 8, 8, 8 };
class Coordinate {
public:
    int x, y, weight;
...
public:
	...
	ll minimax(Coordinate coor, int depth, ll alpha, ll beta, int color, Coordinate& ans)
	{
		if (depth == 0) return 0;
		ll value;
		if (color == ai_side) {
			...
			for ( int i = 0; i < WIDTH[depth] and i <= int(blank.size()); ++i, ++it) {
				...
				Coordinate t(-1, -1);
				tmp += minimax( cur, depth - 1, alpha - tmp, beta - tmp, 1 - color, t);
				if (tmp > value) {
					value = tmp;
					ans = cur;
				}
	...
	std::pair<int, int> solve_change_white() {
		Coordinate t(-1, -1);
		if (value_flip(ai_side) - minimax(Coordinate(-1, -1), 11, INF, NEG_INF, ai_side, t) < minimax(Coordinate(-1, -1), 12, INF, NEG_INF, ai_side, t)) {
	...
	std::pair<int, int> solve_change_black() {
		...
		for (int i = 0; i < 15; ++i)
			for (int j = 0; j < 15; ++j)
				if (board.board[i][j] == -1){
					Coordinate t(-1, -1);
					ll tmp = abs(minimax(Coordinate(-1, -1), 6, INF, NEG_INF, ai_side, t));
					if (tmp < min) {
						tmp = min;
						ans = Coordinate(i, j);
					}
...
std::pair<int, int> action(std::pair<int, int> loc) {
    ...
    Coordinate ans(0,0);
    if (turn == 2 and ai_side == 0) {
		return myAI.solve_change_black();
    }

    for (int i = 2; i <= 12; i += 2) {
        if ( abs(myAI.minimax(Coordinate(-1, -1), i, NEG_INF, INF, ai_side, ans)) > 1000000) {
            {
